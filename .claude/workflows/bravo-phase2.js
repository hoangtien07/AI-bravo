...
phase('Build')
const BUILD = [
  { label: 'build:api', agentType: 'bravo-rag-backend',
  ...
  { label: 'build:judge+analytics', agentType: 'bravo-eval',
  ...
  { label: 'build:suggest', agentType: 'bravo-retrieval-quality',
  ...
  { label: 'build:deploy', agentType: 'bravo-devops',
...
Sau khi sửa, KHÔNG chạy eval (để pha Integrate chạy). Báo chính xác đã sửa gì + vì sao không phá default.`,
  { label: 'tenant:seam', phase: 'Tenant seam', agentType: 'bravo-multitenant' })
...
Báo: con số eval trước (55/56) vs sau, file/endpoint mới, dep đã cài, lỗi đã sửa, rủi ro còn lại.`,
  { label: 'integrate:verify', phase: 'Integrate+Verify', agentType: 'bravo-rag-backend' })
...
const review = await agent(
  ...
  { label: 'review:adversarial', phase: 'Review', agentType: 'bravo-reviewer' })
